...
typedef  void (*GSourceCallbackRefFunc)   (gpointer     cb_data);
typedef  void (*GSourceCallbackUnrefFunc) (gpointer     cb_data);
typedef  void (*GSourceCallbackGetFunc)   (gpointer     cb_data,
                 GSource     *source, 
                 GSourceFunc *func,
                 gpointer    *data);
...
{
    GSourceCallbackRefFunc ref;
    GSourceCallbackUnrefFunc unref;
    GSourceCallbackGetFunc get;
};

typedef void (*GSourceDummyMarshal) (void);

typedef gboolean (*GSourcePrepareFunc)  (GSource *source, gint *timeout_);
typedef gboolean (*GSourceCheckFunc)    (GSource *source);
typedef gboolean (*GSourceDispatchFunc) (GSource *source, GSourceFunc callback, gpointer user_data);
typedef void     (*GSourceFinalizeFunc) (GSource *source); /* Can be NULL */

struct _GSourceFuncs
{
  GSourcePrepareFunc prepare;
  GSourceCheckFunc check;
  GSourceDispatchFunc dispatch;
  GSourceFinalizeFunc finalize;
...
guint    g_source_count (GMainContext *context);
...
#ifdef _WIN64
GLIB_VAR_64 GSourceFuncs g_timeout_funcs;
GLIB_VAR_64 GSourceFuncs g_child_watch_funcs;
GLIB_VAR_64 GSourceFuncs g_idle_funcs;
#else
